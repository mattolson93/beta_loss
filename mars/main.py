...
def test_open_set_performance(model, testing_dataset, openset_dataset, mode, get_acc=False):
    ...
    with torch.no_grad():
        for images, labels in tqdm(testing_dataset):
        ...
        for images, labels in tqdm(openset_dataset):
            logits = model(images.to(device))
...
eps = 1e-8
def kliep_loss(logits, labels, max_ratio=10):
    logits = torch.clamp(logits,min=-25, max=25)
    preds  = torch.sigmoid(eps+logits) * max_ratio
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    criterion = nn.CrossEntropyLoss().cuda(args.gpu) if args.loss == 'ce' else kliep_loss
    ...
    valdir = os.path.join(args.data, 'val_inlier')
    opendir = os.path.join(args.data, 'val_outlier')
    ...
    open_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(opendir, transforms.Compose([
            transforms.Resize(256),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            normalize,
    ...
    if args.evaluate:
        test_open_set_performance(model, val_loader, open_loader, 'kliep')
        test_open_set_performance(model, val_loader, open_loader, 'ce')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if is_best: 
                torch.save(model.state_dict(), os.path.join("outmodels", f'best_{args.arch}_{args.loss}.pth.tar'))
