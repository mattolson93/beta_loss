...
def main():
    ...
    #0.94569 power ratio no wd
    bestacc = 0
    best_auroc = 0
    not_saved = False
    outstr =""
    for score_func in scoring_func:
        ...
        auroc_val_included      = calc_auroc(np.concatenate([id_test_results,validation_results]), ood_test_results)
        tnrATtpr95_val_included = calc_tnr(np.concatenate([id_test_results,validation_results]), ood_test_results)

        if auroc_val_included > best_auroc:
            best_mode = score_func
            best_auroc = auroc_val_included
            best_tnr = tnrATtpr95_val_included
        outstr += f",{auroc_val_included},{tnrATtpr95_val_included}" 
        if not_saved:
            not_saved = False
            if not os.path.exists(out_zs_dir): os.makedirs(out_zs_dir)
            open_labs = np.array([num_classes] * open_zs.shape[0])
            _,  train_labs,  train_zs  = testData_getinfo(model, device, train_data, 0, criterion, score_func, title = 'Validating')

            np.save(os.path.join(out_zs_dir,"train_zs.npy"),train_zs)
            np.save(os.path.join(out_zs_dir,"val_zs.npy"),val_zs)
            np.save(os.path.join(out_zs_dir,"test_zs.npy"),test_zs)
            np.save(os.path.join(out_zs_dir,"open_zs.npy"),open_zs)
            np.save(os.path.join(out_zs_dir,"train_labs.npy"),train_labs)
            np.save(os.path.join(out_zs_dir,"val_labs.npy"),val_labs)
            np.save(os.path.join(out_zs_dir,"test_labs.npy"),test_labs)
            np.save(os.path.join(out_zs_dir,"open_labs.npy"),open_labs)

    best_str = f'{seed},{bestacc},{best_mode},{best_auroc:.4f},{best_tnr:.4f}'
    outstr = best_str + outstr
    
    out_file = os.path.join("results",f'{dataset_name}_{layer_type}_{loss_type}.csv')
    ...
    for score_func in scoring_func:    
        ...
        for noise_magnitude in noise_magnitudes:
            ...
            validation_results    = testData(model, device, val_data , noise_magnitude, criterion, score_func, title = 'Validating')
            cur_val = validation_results.mean()
            if best_val_score is None or cur_val > best_val_score:
                best_mag = noise_magnitude
                best_val_score = cur_val


        acc, id_test_results  = testData(model, device, test_data, best_mag, criterion, score_func, title = 'Testing ID', get_acc=True) 
        ood_test_results      = testData(model, device, open_data, best_mag, criterion, score_func, title = 'Testing OOD')

        
        auroc       = calc_auroc(id_test_results, ood_test_results)
        ...
        if auroc > best_auroc:
            ...
            best_auroc = auroc
            best_tnr = tnrATtpr95
        outstr += f",{auroc},{tnrATtpr95}" 

    best_str = f'{seed},{bestacc},{best_mode},{best_auroc:.4f},{best_tnr:.4f}'
    outstr = best_str + outstr
    
    out_file = os.path.join("results_mag",f'{dataset_name}_{layer_type}_{loss_type}.csv')
    with open(out_file, "a") as writer:
        writer.write(outstr+"\n")    
...
def testData_getinfo(model, CUDA_DEVICE, data_loader, noise_magnitude, criterion, score_func = 'h', title = 'Testing', get_acc=False):
    ...
    if get_acc:
        accuracy = float(total_correct) / total
        print('Test Accuracy: {}/{} ({:.03f})'.format(total_correct, total, accuracy))
        return accuracy,  np.array(results),  np.array(labs).squeeze(), zs.numpy()
    return np.array(results),  np.array(labs).squeeze(), zs.numpy()
