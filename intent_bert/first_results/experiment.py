...
loss_type = sys.argv[5] #kliep, ce, original
...
for _ in trange(int(num_train_epochs), desc="Epoch"):
    ...
    for step, batch in enumerate(tqdm(train_labeled_dataloader, desc="Iteration (labeled)")):
        ...
        loss, logits  = model(input_ids, segment_ids, input_mask, u, l, 'train', label_ids)
        ...
        if loss_type == 'kliep':
            ...
            for i in range(logit_count):
                ...
                loss+= .5*(1/logit_count) * (preds[lbl_idx][:,logit_index].mean(1) -torch.log(preds[lbl_idx][:,i])).mean()
        elif loss_type == 'ce':
            loss += .5*criterion(logits, label_ids)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            tmp_eval_loss, _ = model(input_ids, segment_ids, input_mask, u, l, 'train', label_ids)
...
with open(f"results.csv", "a") as writer:
    ...
    ari = results["ARI"]
    acc = results["ACC"]
    writer.write(f"{seed}, {task_name}, {fraction}, {labeled_ratio}, {unknown_cls_ratio}, {loss_type}, CDAC-KM, {nmi}, {ari}, {acc} \n")
