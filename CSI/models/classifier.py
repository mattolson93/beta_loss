...
from dre_utils import RatioLayer, RatioLayer2, RatioLayerDre, SoftplusLinearDre
...
def get_shift_classifer(model, K_shift):
    ...
    model = model.set_shift_cls_layer(K_shift)

    return model

def get_pred_layer(model, layer_type, n_classes=10):
    #import pdb; pdb.set_trace()
    if layer_type == "ratio":
        model.linear = nn.Linear(model.last_dim, num_classes)
        model.joint_distribution_layer = nn.Linear(model.last_dim, 4 * num_classes)
    else:
        model.linear = nn.Linear(model.last_dim, num_classes)
        model.joint_distribution_layer = nn.Linear(model.last_dim, 4 * num_classes)
...
def get_classifier(mode, n_classes=10):
    ...
    elif mode == 'resnet18ratio':
        classifier = ResNet18(num_classes=n_classes, pred_layer=RatioLayer)
    elif mode == 'resnet18ratio2':
        classifier = ResNet18(num_classes=n_classes, pred_layer=RatioLayer2)
    elif mode == 'resnet18ratiodre':
        classifier = ResNet18(num_classes=n_classes, pred_layer=RatioLayerDre )
    elif mode == 'resnet18spr':
        classifier = ResNet18(num_classes=n_classes, pred_layer=SoftplusLinearDre )
    elif mode == 'resnet34':
