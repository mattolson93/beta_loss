...
def train(P, epoch, model, criterion, optimizer, scheduler, loader, logger=None,
    ...
    if epoch == 1:
        ...
        alpha = torch.nn.parameter.Parameter(torch.tensor(0.0, requires_grad=True).to(device)) 
        beta = torch.nn.parameter.Parameter(torch.tensor(0.0, requires_grad=True).to(device))
        ab_optimizer =  torch.optim.SGD([alpha,beta], lr = .10, momentum = 0.0, weight_decay = 0)
        P.ab = (alpha,beta)
        P.ab_optimizer = ab_optimizer
        P.ab_scheduler = lr_scheduler.MultiStepLR(P.ab_optimizer, gamma=0.1, milestones=milestones)

        linear_optim = torch.optim.SGD(linear.parameters(),
                                       lr=P.lr_init, weight_decay=P.weight_decay)
        ...
        joint_linear_optim = torch.optim.SGD(model.parameters(),
                                             lr=P.lr_init, weight_decay=P.weight_decay)
    ...
    for n, (images, labels) in enumerate(loader):
        ...
        loss_ce = criterion(outputs, labels,alpha,beta) if P.lambd > 0 else criterion(outputs, labels)
        loss_rot = criterion(outputs_rot, rot_labels,alpha,beta) if P.lambd > 0 else  criterion(outputs_rot, rot_labels)
        loss_joint = criterion(outputs_joint, joint_labels,alpha,beta) if P.lambd > 0 else  criterion(outputs_joint, joint_labels)
        loss_ab    = P.lambd * (alpha +beta+2)

        clip = 5
        ### CE loss ###
        P.ab_optimizer.zero_grad()
        P.linear_optim.zero_grad()
        loss_ce.backward()
        torch.nn.utils.clip_grad_norm_(linear.parameters(),clip)
        P.linear_optim.step()

        ### Rot loss ###
        P.rotation_linear_optim.zero_grad()
        loss_rot.backward()
        torch.nn.utils.clip_grad_norm_(rotation_linear.parameters(),clip)
