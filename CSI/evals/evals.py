    ...
    for ood_score in ood_scores:
        ...
        for ood, ood_loader in ood_loaders.items():
            if ood == 'interp':
                ...
                auroc_dict[ood][ood_score] = (get_auroc(scores_id, scores_ood), calc_tnr(scores_id, scores_ood))
...
def compute_ood_score(P, model, ood_score, x, simclr_aug=None):
    ...
    if ood_score == 'clean_norm':
        ...
        score = output_aux['penultimate'].norm(dim=1)
        return score
    ...
    elif ood_score == 'baseline_m':
        outputs, outputs_aux = model(x, penultimate=True)
        #outputs = model.joint_distribution_layer.inlier(outputs_aux['penultimate'])
        scores = outputs.max(dim=1)[0]
        return scores

    elif ood_score == 'baseline_n':
        outputs, outputs_aux = model(x, penultimate=True)
        #outputs = model.joint_distribution_layer.inlier(outputs_aux['penultimate'])
        scores = torch.norm(outputs,p=1 ,dim=1)
        return scores
        ...
        for i in range(4):
            ...
            #total_outputs += model.joint_distribution_layer.inlier(outputs_aux['penultimate'])[:, P.n_classes * i:P.n_classes * (i + 1)]

        scores = F.softmax(total_outputs / 4., dim=1).max(dim=1)[0]
        return scores
    elif ood_score == 'baseline_max':
        ...
        scores = (total_outputs /4. ).max(dim=1)[0]
        return scores
    elif ood_score == 'baseline_norm':
        ...
        scores = torch.norm(total_outputs,p=1 ,dim=1)
        return scores
