...
class Losses:
    ...
    def trapz_loss(self, logits, labels, a, b):
        probs = self.last_sigmoid(logits)
    ...
    def trapz_loss_softmax(self, logits, labels, a, b):
    ...
    def bce(self, input, labels): 
        if type(input) is tuple: input = input[0]
    ...
    def get_loss_dict(self):
        return {
            ...
            'trapz': self.trapz_loss,
            'trapz_softmax': self.trapz_loss_softmax,
...
class GodinLayerTrapz(nn.Module):
    def __init__(self, in_features, num_classes):
        super(GodinLayer, self).__init__()
...
class NormalLinear(nn.Module):
    ...
    def get_modes(): return ['latent_norm2']
...
class SoftplusLinear(nn.Module):
    ...
    def forward(self, x, clampval=50):
        ...
        x = self.h(x)
        if self.training:
            x = dclampp(x, min=-clampval, max=clampval)
