...
def get_args():
    ...
    parser.add_argument('--lambd', type=float, default=0.0, help='beta of the kl in loss function')
    ...
    parser.add_argument('--flayer', type=str, default="linear", help='final layer of class prediction')
    parser.add_argument('--loss', type=str,  default="ce", help='class prediction loss')
...
def train(args, lvae):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in  enumerate(tqdm(train_loader)):
            ...
            optimizer.zero_grad()
            if args.lambd > 0: loss += (args.losses.alpha +args.losses.beta+2) * args.lambd
            loss.backward()
            if args.lambd > 0:
                torch.nn.utils.clip_grad_norm_(lvae.parameters(),40)
                with torch.no_grad():
                    args.losses.alpha.clamp_(-1,0)
                    args.losses.beta.clamp_(-1,0)
...
if __name__ == '__main__':
    ...
    args.losses = my_losses

    ### run experiment 1/5 times
    for run_idx in range(0, args.exp+1):
        ...
        lvae = LVAE(in_ch=in_channel,
                    ...
                    num_class=args.num_classes, dataset=args.dataset,
                    final_layer=selected_layer,classification_loss=criterion, args=args)
        ...
        # optimzer
        optimizer = optim.SGD(list(lvae.parameters()) +[my_losses.alpha,my_losses.beta], lr=args.lr, momentum=args.momentum, weight_decay=args.wd)
