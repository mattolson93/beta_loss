...
class Losses:
    def __init__(self, do_softplus=True):
        ...
        #self.max_clip = torch.tensor(0, dtype=torch.float).cuda()
        self.max_clip = torch.tensor(1000, dtype=torch.float).cuda()
        self.alpha = torch.nn.parameter.Parameter(torch.tensor(0.0, requires_grad=True).cuda()) 
        self.beta = torch.nn.parameter.Parameter(torch.tensor(0.0, requires_grad=True).cuda())
    ...
    def trapz_loss(self, logits, labels, a = -.5, b=-.5):
        ...
        lower_preds = preds * torch.linspace(eps,one_m_eps,num_tzoids).unsqueeze(1).unsqueeze(1).cuda()
        upper_preds = 1- ((1-preds) * torch.linspace(one_m_eps,eps,num_tzoids).unsqueeze(1).unsqueeze(1).cuda())
    ...
    def trapzab_softmax(self, logits, labels):
        a = self.alpha
        b = self.beta
    ...
    def trapzab_sigmoid(self, logits, labels):
        a = self.alpha
        b = self.beta
...
class SoftplusLinear(nn.Module):
    ...
    def forward(self, x, clampval=50):
        x = self.h(x)
        if self.training:
            x = torch.clamp(x, min=-clampval, max=clampval)
...
class RatioLayerDre(nn.Module):
    ...
    def forward(self, x, max_logit=15.942385):
        self.z = x
        i = self.inlier(x) #/ self.in_featuressqrt
        o = self.outlier(x) #/ self.in_featuressqrt

        if self.training:
            i = dclamp(i, min=-max_logit, max=max_logit)
            o = dclamp(o, min=-max_logit, max=max_logit)
       

        logits = torch.sigmoid(i) / (torch.sigmoid(o)+ self.eps)
        if self.training: 
            logits = dclamp(logits, min = 1.1920929e-07, max=500)
...
def get_exp_name(opt):
    cf_option_str = [opt.dataset, opt.loss, opt.flayer]
    ...
    if opt.lambd:
        additional_str = 'betalambd%s' %opt.lambd
        cf_option_str.append(additional_str)
